...
2. Path A is done (not "next"). Remaining, ranked by what's actually been measured: SwiGLU
   (14.2ms, deferred pending quality validation beyond single-prompt coherence), a real
   router-as-HP-Lin test through `lin_mm_hp` (the fp16 experiment above tested a different,
   single-threaded code path — not yet conclusive, see that row), and another look at `linear`
   itself now that A3's 446ns/call kernel-only number is 6+ tok/s old.
