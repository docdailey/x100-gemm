...
| 2026-07-26 | A5 (real) | 3131fef | `qwen_moe_hp.c` full engine integration (see A2-corrected/A3 rows), run against the real 30B-A3B model, nt=4 | tok/s + correctness | **6.19 tok/s, ' Tokyo' PASS, coherent generation matches the original engine exactly.** Up from our 1.49 tok/s (**4.16x**), still below the real vendor binary's 11.71-12.89 (A5-equiv). Requant into the new format took **1104.5s (~18.4 min)** — much slower than the old format's ~2 min parallel requant (real cost: more fp16 conversions + 8-subblock nesting per weight group); cache save 92.8s. **Buckets are the real finding**: `linear(kernel)` 575.9→**38.4ms** (15x, as A3 predicted) but `rest` ballooned to **100.3ms = 62% of wall** (was 59.7ms/9% before). Almost certainly OpenMP fork-join overhead — `lin_mm_hp` spawns a fresh `#pragma omp parallel` team per Lin call, 29/layer x 48 layers = 1392 team spawns/token, and now that the kernel itself is ~446ns/call the fixed per-spawn cost dominates. This is exactly `codex_recs_1.md` §17 PR8 ("persistent pool, if still warranted") — now warranted with real data. If that overhead collapses, projected wall-clock (~60ms/tok from kernel+attn+pack alone) would **beat** the vendor binary's 12.89 tok/s. |
