...
| 2026-07-26 | A5 (real) | (pending) | `qwen_moe_hp.c` — full engine integration: `gemm_kernel_i8i4_hp_m1` swapped in for `gemv_nb_int4_grouped` across all 8 Lin sites (q/k/v/o/eg/eu/ed/lm), P0.2 shared-activation-pack structure preserved (`Abuf2` for q/k/v and eg/eu, transient `Abuf` for o/ed/lm). All this model's Lin shapes (d=2048, qd=4096, kvd=512, moe_ffn=768, vocab=151936) are exact multiples of 256(K)/32(N) — zero remainder handling needed. New incompatible cache format (`IMEC` ver=2, separate path `qwen3-30b-a3b.hp.imecache`) since the on-disk weight layout changed entirely (vendor N32-panel/K256-superblock, not our q4-in-q8 interleave) — kept as a **separate binary next to `qwen_moe.c`**, not a runtime flag, so the original working/committed engine is never at risk. First full requant + coherence + tok/s run in progress. | in progress | Full 30B-A3B requant into the new format running now (48 layers, 8-way parallel over layers, same structure as the original engine's parallel requant). Row will be updated with tok/s and ' Tokyo' PASS/FAIL once it completes. |
