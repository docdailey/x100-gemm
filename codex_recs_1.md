...
**Decision (explicit, given verbatim): "Record simple n-gram speculation as closed for
general-purpose decode: the acceptance oracle failed its predeclared gate, and no integrated M4
verifier should be built. Preserve the oracle as a diagnostic/research tool. Do not implement a
repetition-only production fast path yet; its projected ~1.12× niche gain is too small relative to
implementation and validation cost."**

**Track CLOSED.** No batched-verify mechanism, general or repetition-gated, will be built on top of
this oracle. `ngram_propose`/`spec_generate_trajectory`/`run_speculative_oracle`
(`QWEN_SPEC_ORACLE=1`) remain in the codebase as a standing diagnostic/research tool — re-runnable
if the calculus changes later (a cheaper verify mechanism, a different drafting strategy, or a
workload with enough genuine repetition to justify the gated fast path's own implementation cost),
but nothing further is scheduled. Single-user M=1 production behavior is completely untouched by
this track (the oracle only ever reads real trajectories generated by the existing, unmodified
`forward()` path; no production code path was added or changed). Per the user's own explicit
direction, next: design work for larger-N continuous serving (workload goals, N sweep, memory/KV
budget, latency constraints, and an expert-overlap oracle) — BEFORE any production code changes.

### 22.41 Larger-N continuous serving: design phase (workload goals, hardware ceiling, memory/KV budget, latency cost) — BEFORE any production code

Explicit direction: "Proceed to design the larger-N continuous-serving track, beginning with
workload goals, N sweep, memory/KV budget, latency constraints, and an expert-overlap oracle before
changing production code. Keep single-user M=1 behavior untouched." This section covers the four
analytical dimensions using real, already-measured numbers; the expert-overlap oracle (the one
dimension requiring new instrumentation) follows as its own subsection.

**Workload framing (stated assumption, not yet confirmed with the user).** This board runs a single
model instance for what this whole session has treated as fundamentally single-user/personal-research
use, with M-batching investigated as an efficiency lever when requests happen to coincide — not a
public multi-tenant service. The most plausible reading of "aggregate multi-user throughput": several
concurrent AGENT SESSIONS or batch/eval jobs hitting the same board at once (a realistic shared-
research-backend scenario), not large-scale public serving. N is therefore assumed to mean low
single digits to low tens of concurrent streams, not hundreds — flagged explicitly since it changes
which of the two architectures below is worth building.

**The hardware ceiling this design must account for, found by re-examining the already-committed
hart allocation (not a new measurement — reading `main()`'s own existing `g_lin_nt`/`g_attn_nt`
setup).** Every prior M-batch milestone (1-3, batched prefill) ALL 8 A100 harts are already
committed to keeping ONE M4-wide round moving as fast as possible: linear/IME uses up to 4
(`g_lin_nt=min(nt,4)`), attention uses up to 8 (`g_attn_nt`, Phase 6, §22.33). There is no spare hart
capacity to run a second, fully independent M4 dispatch truly IN PARALLEL under this allocation —
which means "larger N" cannot come from raw added compute parallelism unless the hart allocation
itself is redesigned. This surfaces a genuine architectural fork worth deciding before any N-sweep
measurement is built:

- **(A) Single M4-lane continuous batching** — extend milestones 1-3's existing mechanism (already
  built, validated, currently exercised only at a fixed, artificially-favorable N=4-co-terminating-
  sequences test) with REAL scheduling: N pending sequences queue, and whenever the one active
  4-wide M4 lane has a free slot (a sequence hit EOS or hasn't started yet), a new pending sequence
  fills it immediately — the actual point of "continuous" batching, avoiding the idle-slot/shrink-to-
  3 problem a naive fixed-batch-of-4 would hit. Throughput ceiling is bounded by how well the
  scheduler keeps the ONE lane's 4 slots genuinely useful, not by N itself past the point queueing
  depth is enough to keep slots full.
- **(B) Hart-partitioned multi-lane** — split the 8 A100 harts into 2+ independent single-sequence M1
  lanes (e.g. 2 lanes × 4 harts each) instead of 1 lane × 8 harts. Each lane runs the EXISTING,
  unmodified, already-proven M1 decode path — genuinely parallel, no M4 batching, no quantization
  approximation at all (bit-exact, unlike A). Trades single-lane peak speed (today's 12.4 tok/s
  headline needs the full 8-hart attention allocation) for true concurrency. Not yet measured how
  attention/linear scale down at 4 harts instead of 8 — this is itself a first, cheap thing to check
  before deciding between A and B.

**Latency cost of (A), from milestone 2's own already-measured numbers (re-derived here, not
re-run): a sequence sharing a 4-wide M4 round pays roughly 3.6-3.7x a solo M1 step's latency for its
own next token** (milestone 2: 16 rounds of batched M=4 decode cost 5.567s total → 0.348s/round;
16 rounds of 4 separate M1 decodes cost 5.987s total → 0.0935s per single-sequence step; ratio
0.348/0.0935 ≈ 3.72x — consistent with the ~3.6x figure independently derived for the speculative-
decoding cost model in §22.40). This is the fundamental trade continuous batching makes under THIS
hardware: each participant's own token-to-token latency roughly quadruples whenever it shares a
round with 3 others, in exchange for milestone 2's own measured ~1.11x aggregate throughput gain —
a much steeper latency cost than GPU-style batching typically pays, because this CPU/RVV engine's
M4 arithmetic genuinely scales with M (§22.36) rather than batching being close to free. Any N-sweep
or scheduler design for (A) needs an explicit, stated bound on acceptable per-stream latency
degradation, not just an aggregate-throughput target — a config that maximizes aggregate tok/s by
keeping every sequence in a permanently-full 4-wide lane could make EVERY individual stream feel
~3.6x slower than today's solo default, which may not be the right trade for interactive use.

**Memory/KV budget, real numbers (checked fresh on the board, not from stored memory).** Total RAM
31 GiB (`free -h`); resident weight set ≈17.0 GiB (imecache file size, 18,288,076,356 bytes — RSS
observed directly earlier this session at ~17.7 GiB, consistent); headroom ≈13-14 GiB. Budgeting
~10 GiB conservatively for KV caches + scratch + OS margin. Per-sequence KV cache size is exactly
`2 × nl × kvd × 4 bytes × ctx` = `2×48×512×4×ctx` = 196,608 bytes/position = **192 KiB/position**:

| context length | KV cache / sequence | max sequences at ~10 GiB budget |
|---|---|---|
| 512 | 96 MiB | ~106 |
| 1024 | 192 MiB | ~53 |
| 2048 | 384 MiB | ~26 |
...
At realistic agentic/chat context lengths (2-8K), memory alone permits somewhere between ~6 and ~26
concurrent sequences — comfortably above the low-single-digit-to-low-tens N this design assumes is
the real workload target, meaning **memory is very unlikely to be the binding constraint**; the
scheduling/hart-allocation questions above are the real design problem, not KV capacity.

**N sweep, reframed given the hardware-ceiling finding above.** For architecture (A), "N sweep"
should NOT mean "run N=4/8/16/32 simultaneously" (there's no spare parallelism for that under the
current 8-hart-single-lane allocation) — it means testing how well a scheduler keeps the ONE M4 lane
full as REQUEST ARRIVAL/COMPLETION DYNAMICS vary (sequences starting/finishing at different times,
unlike milestones 1-3's artificially-synchronized fixed-4 test), at increasing QUEUE DEPTH N. For
architecture (B), "N sweep" means testing how many independent hart-partitioned lanes (2? 4?) the
board can usefully run, bounded by how much attention/linear slow down per lane at a reduced hart
count — not yet measured, proposed as the first cheap check before committing to (B).

**Proposed immediate next step (not yet started): the expert-overlap oracle**, extending
milestone 3's own N=4 same-expert-overlap measurement (§22.37) to larger, REALISTIC pending-queue
sizes — directly relevant to architecture (A)'s scheduling design (if a scheduler can choose WHICH 4
of N pending sequences fill the next round, picking ones that share experts could meaningfully raise
